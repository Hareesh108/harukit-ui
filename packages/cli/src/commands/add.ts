...
export async function add(components: string[], options: any) {
  ...
  try {
    ...
    if (componentsToAdd.length === 0) {
      spinner.fail(
        "Please specify components to add. Example: npx harukit@latest add button card"
      );
      process.exit(1);
    }
    ...
    for (const componentMeta of componentMetas) {
      componentMeta.dependencies.forEach((dep: string) =>
        allDependencies.add(dep)
      );
      componentMeta.devDependencies.forEach((dep: string) =>
    ...
    if (allDependencies.size > 0 || allDevDependencies.size > 0) {
      ...
      try {
        ...
        if (allDependencies.size > 0) {
          ...
          console.log(
            chalk.cyan(`→ Installing dependencies: ${depsArray.join(", ")}`)
          );
        ...
        if (allDevDependencies.size > 0) {
          ...
          console.log(
            chalk.cyan(
              `→ Installing dev dependencies: ${devDepsArray.join(", ")}`
            )
          );
    ...
    for (const componentMeta of componentMetas) {
      for (const file of componentMeta.files) {
        ...
        if ((await fs.pathExists(destPath)) && !options.overwrite) {
          console.log(
            chalk.yellow(
              `⚠️  ${file.name} already exists. Use --overwrite to replace.`
            )
          );
          continue;
        }
        ...
        try {
          ...
          console.log(
            chalk.red(`❌ Failed to add ${file.name}: ${getErrorMessage(err)}`)
          );
        }
      }
      ...
      if (componentMeta.dependencies.length > 0) {
        console.log(
          chalk.blue(
            `   Dependencies: ${componentMeta.dependencies.join(", ")}`
          )
        );
      }
    }
    ...
    console.log(
      chalk.green(
        "2. Add more components with: npx harukit@latest add <component>"
      )
    );
